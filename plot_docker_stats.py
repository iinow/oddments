...
def main():
    ...
    ap.add_argument('--simdjson', default='')
    ...
    sx, scpu, smem = ([], [], [])
    if args.simdjson:
        sp = Path(args.simdjson)
        if sp.exists():
            sx, scpu, smem = load_stats(sp)
    ...
    if sx:
        ax[0].plot(sx, scpu, label='SimdJson', color='#59a14f')
    ...
    if sx:
        ax[1].plot(sx, smem, label='SimdJson', color='#59a14f')
